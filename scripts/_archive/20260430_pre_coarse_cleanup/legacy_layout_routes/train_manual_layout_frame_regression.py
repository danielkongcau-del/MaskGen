...
from partition_gen.manual_layout_frame import (  # noqa: E402
    ...
    evaluate_layout_frame_regressor,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train v2b continuous regression layout/frame baseline.")
...
def save_checkpoint(path: Path, *, model, optimizer, model_config, tokenizer_config, train_config, metrics, epoch: int) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    torch.save(
        {
            "format": "maskgen_manual_layout_frame_regression_checkpoint_v1",
...
def main() -> None:
    ...
    run_name = args.run_name or time.strftime("frame_regression_%Y%m%d_%H%M%S")
    ...
    model_config = ManualLayoutFrameRegressorConfig(
    ...
    model = ManualLayoutFrameRegressor(model_config).to(device)
    ...
    train_config.update(
        {
            ...
            "baseline_role_label_origin_mae": 105.45383331783475,
        }
    )
    save_json(
        output_dir / "config.json",
        {"model_config": asdict(model_config), "tokenizer_config": asdict(tokenizer_config), "train_config": train_config},
    )
    ...
    for epoch in range(1, int(args.max_epochs) + 1):
        ...
        if epoch % int(args.eval_every) == 0 or epoch == int(args.max_epochs):
            metrics = evaluate_layout_frame_regressor(model, val_loader, device=device, config=tokenizer_config)
