...
def summarize(rows: List[dict]) -> str:
    ...
    lines.extend(["", "## Evidence Reference Encoding Groups"])
    for profile in evidence_ref_profiles:
        profile_rows = [row for row in ok_rows if str(bool(row.get("token_encode_evidence_refs", False))) == profile]
        lines.extend(
            [
                "",
                f"### token_encode_evidence_refs={profile}",
                "",
                f"- samples: {len(profile_rows)}",
                f"- valid ratio: {_ratio(profile_rows, 'validation_is_valid'):.3f}",
                f"- mean residual_area_ratio: {_mean(profile_rows, 'residual_area_ratio'):.6f}",
    ...
    lines.extend(
        ...
        "## Top False Cover",
        "",
        _table(_top(ok_rows, "false_cover_ratio_max"), ["source", "independent_baseline_profile", "token_encode_evidence_refs", "false_cover_ratio_max", "false_cover_area_total", "hard_false_cover_candidate_count", "validation_is_valid"]),
        "",
        "## Top Residual Area Ratio",
        "",
        _table(_top(ok_rows, "residual_area_ratio"), ["source", "independent_baseline_profile", "token_encode_evidence_refs", "residual_area_ratio", "residual_face_count", "operation_histogram"]),
        "",
        "## Lowest Compression Gain",
        "",
        _table(_top(ok_rows, "total_compression_gain", reverse=False), ["source", "independent_baseline_profile", "token_encode_evidence_refs", "total_compression_gain", "residual_area_ratio", "operation_histogram"]),
        ]
    )
    return "\n".join(lines) + "\n"
