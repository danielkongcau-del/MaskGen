...
from partition_gen.manual_geometry_conditioning import build_conditioned_geometry_sequence_rows  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Tokenize topology-conditioned manual geometry training sequences.")
...
def main() -> None:
    ...
    rows, summary = build_conditioned_geometry_sequence_rows(
        args.split_root,
        config=config,
        vocab=vocab,
    ...
    sequences_path = args.output_root / "conditioned_geometry_sequences.jsonl"
        ...
        for row in rows:
            ...
            write_jsonl_row(
                manifest_handle,
                {
                    ...
                    "geometry_length": row.get("geometry_length"),
    ...
    print(
        f"tokenized conditioned geometry samples={summary['sample_count']} "
        f"sequences={summary['written_conditioned_geometry']} "
