...
from partition_gen.boundary_training import compute_boundary_losses
from partition_gen.models.boundary_predictor import build_boundary_model_from_metadata


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train a graph-conditioned shared-boundary predictor.")
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("outputs/boundary_predictor"))
    ...
    parser.add_argument("--raster-channels", type=int, default=96)
    parser.add_argument("--scene-channels", type=int, default=32)
...
def make_loader(
    *,
    ...
    return DataLoader(
        ...
        collate_fn=collate_boundary_graphs,
...
def evaluate(model, loader: DataLoader, device: torch.device, use_amp: bool, *, pos_weight: torch.Tensor | None) -> Dict[str, float]:
    totals = {
        "total": 0.0,
        "bce": 0.0,
        "dice": 0.0,
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            with autocast_context(device, use_amp):
                ...
                losses = compute_boundary_losses(outputs, batch, pos_weight=pos_weight)
            for key in totals:
...
def maybe_run_eval(
    *,
    ...
    metrics = evaluate(model, val_loader, device, use_amp, pos_weight=pos_weight)
    print(
        f"val epoch={epoch} step={global_step} "
        f"loss={metrics['total']:.4f} bce={metrics['bce']:.4f} dice={metrics['dice']:.4f}"
    )
...
def main() -> None:
    ...
    model = build_boundary_model_from_metadata(
        ...
        raster_channels=args.raster_channels,
    ...
    save_json(
        args.output_dir / "boundary_weights.json",
        {
