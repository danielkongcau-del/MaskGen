...
from partition_gen.manual_geometry_sample_validation import (  # noqa: E402
    decode_geometry_tokens_to_target,
    geometry_prefix_tokens,
    validate_geometry_tokens,
)
from partition_gen.manual_topology_generated_geometry import build_generated_geometry_targets_from_sample_rows  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Attach generated geometry from a geometry AR checkpoint to topology samples.")
...
def _make_geometry_sampler(args: argparse.Namespace):
    ...
    def geometry_sampler(node: dict, node_index: int) -> tuple[dict | None, dict]:
        nonlocal request_count
        request_count += 1
        prefix = geometry_prefix_tokens(
        ...
        tokens = [str(token) for token in sample.get("tokens", []) or []]
        validation = validate_geometry_tokens(tokens)
        diagnostics = {
            "valid": bool(validation["valid"]),
            "errors": list(validation["errors"]),
            "length": int(sample.get("length", len(tokens))),
        ...
        if int(args.progress_every) > 0 and request_count % int(args.progress_every) == 0:
            print(f"generated_geometry_request {request_count}")
...
def main() -> None:
    ...
    checkpoint, vocab, constraint_config, geometry_sampler = _make_geometry_sampler(args)
    ...
    for index, target in enumerate(targets):
        ...
        manifest_rows.append(
            {
                "format": "maskgen_generated_geometry_manifest_row_v1",
    ...
    print(
        f"attached generated geometry samples={summary['output_count']} "
